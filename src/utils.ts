...
/**
 * Mapping between user-friendly field names and EMV QR code tag IDs
 */
 ...
 */
export const TAG_TO_FIELD_MAPPING: Record<string, string> =
  Object.entries(FIELD_TO_TAG_MAPPING).reduce((acc, [key, value]) => {
    acc[value] = key;
    return acc;
  }, {} as Record<string, string>);

/**
 * Converts a dictionary with user-friendly field names to EMV QR code tag IDs
 * @param friendlyData Dictionary with user-friendly field names
 * @returns Dictionary with EMV QR code tag IDs
 */
export function mapFriendlyToEmv(friendlyData: Record<string, string>): Record<string, string> {
  const emvData: Record<string, string> = {};

  // Set QR format indicator (always "01")
  emvData["00"] = "01";

  // Handle initiation method conversion (dynamic/static)
  if ("initiation_method" in friendlyData) {
    const method = friendlyData.initiation_method.toLowerCase();
    if (method === "dynamic") {
      emvData["01"] = "11";
  ...
  // Handle currency conversion from alphabetic to numeric code
  if ("currency" in friendlyData) {
    let currencyValue = friendlyData.currency;
    // Check if we have an alphabetic code that needs conversion
    if (/^[A-Za-z]+$/.test(currencyValue)) {
      currencyValue = currencyValue.toUpperCase();
      if (currencyValue in CURRENCY_CODE_MAPPING) {
        friendlyData.currency = CURRENCY_CODE_MAPPING[currencyValue];
      } else {
  ...
  // Map all other fields using the mapping dictionary
  for (const [field, value] of Object.entries(friendlyData)) {
    if (field in FIELD_TO_TAG_MAPPING && field !== "initiation_method") {
      const tagId = FIELD_TO_TAG_MAPPING[field];
      emvData[tagId] = String(value);
    }
  }

  // Handle merchant account information (tags 02-51)
  // This would need additional logic for specific implementations

  return emvData;
}

/**
 * Converts a dictionary with EMV QR code tag IDs to user-friendly field names
 * @param emvData Dictionary with EMV QR code tag IDs
 * @returns Dictionary with user-friendly field names
 */
export function mapEmvToFriendly(emvData: Record<string, string>): Record<string, string> {
  const friendlyData: Record<string, string> = {};

  // Handle initiation method conversion
  if ("01" in emvData) {
    if (emvData["01"] === "11") {
      friendlyData.initiation_method = "dynamic";
    } else if (emvData["01"] === "12") {
      friendlyData.initiation_method = "static";
    }
  }

  // Map other fields using the reverse mapping
  for (const [tagId, value] of Object.entries(emvData)) {
    if (tagId in TAG_TO_FIELD_MAPPING) {
      const field = TAG_TO_FIELD_MAPPING[tagId];

      // Special handling for currency - convert numeric to alphabetic
      if (field === "currency" && value in NUMERIC_TO_CURRENCY_CODE) {
        friendlyData[field] = NUMERIC_TO_CURRENCY_CODE[value];
      } else {
        friendlyData[field] = value;
      }
    }
  }
